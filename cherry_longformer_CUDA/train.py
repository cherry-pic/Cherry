...
def test_model(trained_model_path,test_dataloader,pretrained_model,freeze_pretrained,dropout,device,prediction_threshold,test_df,exp_dir,classification_type):
    # load trained model
...
def calc_valid_loss(net, device, loss_func, dataloader,classification_type):
    ...
    with torch.no_grad():
        for it, (seq, attn_masks, global_attention_mask, labels) in enumerate(tqdm(dataloader)):
            if classification_type > 2:
                labels = labels.type(torch.LongTensor)
            seq, attn_masks, global_attention_mask, labels = seq.to(device), attn_masks.to(device), global_attention_mask.to(device), labels.to(device)
...
def train_for_n_epochs(net, loss_func, opti, lr,lr_scheduler,train_dataloader,val_dataloader, epochs, device, pretrained_model,exp_dir,classification_type):
    ...
    for ep in range(epochs):
        ...
        for it, (seq, attn_masks, global_attention_mask, labels) in enumerate(tqdm(train_dataloader)): #identical to for step, batch in enumerate(train_loder)
            ...
            with autocast():
                ...
                outputs = net(input_ids=seq, attention_mask=attn_masks, global_attention_mask=global_attention_mask)
        ...
        if val_loss < best_loss:
            ...
            path_to_hf_model = exp_dir+'{}_lr_{}_val_loss_{}_ep_{}.hf'.format(modified_pretrained_model_name, lr, round(best_loss, 5), best_ep)
            torch.save(net_copy.state_dict(), path_to_model)
            net_copy.save_pretrained(path_to_hf_model)
...
def train_model(lr, dataloaders, epochs, device, pretrained_model,freeze_pretrained,dropout, hidden_size,prediction_threshold,test_dfs,exp_dir,classification_type):
    ...
    opti = AdamW(net.parameters(), lr=lr, weight_decay=1e-2)  # define optimizer
    ...
    for f,fold in enumerate(dataloaders):
        ...
        test_model(trained_model_path,test_dataloader,pretrained_model,freeze_pretrained,dropout,device,prediction_threshold,test_df,exp_dir,classification_type)
